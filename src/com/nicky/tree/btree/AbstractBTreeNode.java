...
public abstract class AbstractBTreeNode <K extends Comparable<K>> {
    ...
    void insertKey(K key) {
        ...
        while (i > 0 && key.compareTo(this.getKey(i - 1)) < 0) {
            this.setKey(this.getKey(i - 1), i);//如果插入的关键字比前一位关键字小
            i--;
        }
        this.setKey(key, i);//i = 0 直接keys[i]加入关键字
        this.setNKey(this.nkey() + 1); //nkey++
    }
     ...
     * 分裂当前节点
     * 把原结点上的关键字按升序排序后，从中间位置把关键字（不包括中间位置的关键字）分成两部分。
     * 左部分所含关键字放在旧结点中，右部分所含关键字放在新结点中，
     * 中间位置的关键字连同新结点的存储位置插入到父结点中。如果父结点的关键字个数也超过（m-1），
     * 则要再分裂，再往上插。直至这个过程传到根结点为止。
     */
